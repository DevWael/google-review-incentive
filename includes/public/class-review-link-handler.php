	...
	private function schedule_coupon_email( string $customer_email, string $coupon_code ): void {
		...
		if ( function_exists( 'as_schedule_single_action' ) ) {
			as_schedule_single_action(
				...
				array( $customer_email, $coupon_code ),
				'google-review-incentive'
			);

			if ( function_exists( 'wc_get_logger' ) ) {
				$logger = wc_get_logger();
				$logger->info( sprintf( 'Scheduled coupon email for %s to be sent in %d minutes', $customer_email, $delay_minutes ), array( 'source' => 'google-review-incentive' ) );
			}
		}
	}
