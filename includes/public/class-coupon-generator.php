...
class Coupon_Generator
{
    ...
    public function generate_coupon(string $customer_email, WC_Order $order)
    {
        ...
        try {
            ...
            $coupon->set_description(sprintf(__('Thank you coupon for %s (Order #%d)', 'google-review-incentive'), $customer_email, $order->get_id()));
            ...
            if (function_exists('wc_get_logger')) {
                ...
                $logger->info(sprintf('Generated coupon %s for %s (Order #%d)', $coupon_code, $customer_email, $order->get_id()), ['source' => 'google-review-incentive']);
            }
            ...
            if (function_exists('wc_get_logger')) {
                $logger = wc_get_logger();
                $logger->error(sprintf('Failed to generate coupon for %s: %s', $customer_email, $e->getMessage()), ['source' => 'google-review-incentive']);
            }
